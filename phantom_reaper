...
class SecurePortScanner:
    ...
    def _generate_ecc_keys(self):
        # Generate three ECC-528 bit keys for triple encryption
        return [ec.generate_private_key(ec.SECT571R1(), default_backend()) for _ in range(3)]

    def _rotate_keys(self):
        while self.gui_running and self.rotation_interval:
            time.sleep(self.rotation_interval)
            self.ecc_keys = self._generate_ecc_keys()
            logger.info(f"ECC keys rotated at interval: {self.rotation_interval} seconds")
    ...
    def phantom_reaper_connect(self, number, message, chat_widget):
        """Connect to SMS/MMS/RCS/Signal via a unified interface"""
        try:
            # Simplified Signal API integration (actual implementation would need Signal CLI or API)
            logger.info(f"Phantom Reaper sending to {number}: {message}")
            if chat_widget:
                chat_widget.insert(tk.END, f"Phantom Reaper {number}: {message}\n", "sent")
            # Here you would add actual SMS/MMS/RCS API calls (e.g., Twilio for SMS/MMS)
            # For Signal, reuse the signal_store if possible or implement separate Signal CLI calls
        except Exception as e:
            logger.error(f"Phantom Reaper error: {e}")
            if chat_widget:
                chat_widget.insert(tk.END, f"Error: {e}\n", "error")

    def encrypt_email(self, message):
        """Triple encrypt message with ECC-528"""
        encrypted = message.encode()
        for key in self.ecc_keys:
            public_key = key.public_key()
            shared_key = key.exchange(ec.ECDH(), public_key)
            kdf = PBKDF2HMAC(
                algorithm=hashes.SHA256(),
                length=32,
                salt=os.urandom(16),
            ...
            encryptor = cipher.encryptor()
            encrypted = iv + encryptor.update(encrypted) + encryptor.finalize()
        return base64.b64encode(encrypted).decode()

    def send_encrypted_email(self, to_email, subject, message, chat_widget):
        try:
            encrypted_message = self.encrypt_email(message)
            msg = MIMEText(encrypted_message)
            msg['Subject'] = subject
            msg['From'] = "your_email@example.com"  # Replace with your email
            msg['To'] = to_email

            with smtplib.SMTP('smtp.gmail.com', 587) as server:  # Example with Gmail
                server.starttls()
            ...
            if chat_widget:
                chat_widget.insert(tk.END, f"Email to {to_email}: {message}\n", "sent")
        except Exception as e:
            logger.error(f"Email error: {e}")
            if chat_widget:
                chat_widget.insert(tk.END, f"Email error: {e}\n", "error")
    ...
    def run_gui(self):
        ...
        chat_display.grid(row=1, column=0, columnspan=4, padx=5, pady=5)
        ...
        # Remote Connection
        ttk.Label(chat_frame, text="Remote Number/Email:").grid(row=4, column=0, padx=5, pady=5)
        remote_target = ttk.Entry(chat_frame)
        remote_target.grid(row=4, column=1, padx=5, pady=5)
        ...
        ttk.Button(chat_frame, text="Phantom Reaper", command=lambda: threading.Thread(
            target=self.phantom_reaper_connect, args=(remote_target.get(), message_entry.get(), chat_display)).start()
        ).grid(row=6, column=0, pady=5)

        ttk.Button(chat_frame, text="Send Encrypted Email", command=lambda: threading.Thread(
            target=self.send_encrypted_email, args=(remote_target.get(), "Secure Message", message_entry.get(), chat_display)).start()
        ...
        ttk.Label(chat_frame, text="Email Key Rotation:").grid(row=8, column=0, padx=5, pady=5)
        rotation_var = tk.StringVar(value="None")
        rotation_menu = ttk.OptionMenu(chat_frame, rotation_var, "None", "None", "2 min", "3 min", "3 sec",
            command=lambda x: self._start_key_rotation(x))
        rotation_menu.grid(row=8, column=1, pady=5)

        root.protocol("WM_DELETE_WINDOW", lambda: [setattr(self, 'gui_running', False), root.destroy()])
        root.mainloop()

    def _start_key_rotation(self, option):
        intervals = {"None": None, "2 min": 120, "3 min": 180, "3 sec": 3}
        self.rotation_interval = intervals[option]
        if self.key_rotation_thread and self.key_rotation_thread.is_alive():
            self.gui_running = False  # Stop existing thread
            self.key_rotation_thread.join()
            self.gui_running = True
        if self.rotation_interval:
            self.key_rotation_thread = threading.Thread(target=self._rotate_keys)
